...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        ### Using hint, only need first output of self.forward
        prediction,_ = self.forward(obs,acs,obs_mean,obs_std,acs_mean,acs_std,delta_mean,delta_std)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target = ((next_observations-observations)-data_statistics['delta_mean'])/data_statistics['delta_std'] # TODO(Q1) compute the normalized target for the model.
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.
        
        ### Get normalized prediction using self.forward (returns torch)
        _, prediction_normalized = self.forward(obs,acs,obs_mean,obs_std,acs_mean,acs_std,delta_mean,delta_std)

        loss = self.loss(prediction_normalized, ptu.from_numpy(target)) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.

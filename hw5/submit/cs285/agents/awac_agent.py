...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        #!!!
        qa_value = critic.get_qa_values(obs)
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        #!!!
        qa_vals = self.exploitation_critic.q_net(ob_no)
        # action_probs
        v_pi = torch.sum(dist.probs * qa_vals, dim=1)
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            #!!!
            expl_bonus = self.exploration_model.forward_np(ob_no).squeeze()
            if self.normalize_rnd:
                expl_bonus =expl_bonus / self.running_rnd_rew_std
                self.running_rnd_rew_std = self.running_rnd_rew_std*self.rnd_gamma + np.std(expl_bonus)*(1-self.rnd_gamma)
            #!!!
            ...
            actor_loss = self.awac_actor.update(ob_no, ac_na, adv_n=advantage)
            #!!!
